...
4. 우선순위 분류 (이 기준이 단일 출처 — `review` 스킬의 task 우선순위도 이 분류를 그대로 따른다):
   ...
   **검증 게이트 — 🔴(및 버그성 🟡)로 단정하기 전 반드시 통과 (오탐 방지의 핵심)**: diff 패턴만 보고 race/dangling/overflow/crash 를 단정하지 않는다. 보고 직전 스스로 반박을 시도하고(self-refutation), 실제 head 코드 근거로 확인되지 않으면 🔴 로 올리지 않는다.
   ...
   - **확신 없으면 단정 금지**: 코드 근거로 확정 못 하면 🔴 금지 — 한 단계 낮추거나(🟡/🟢), 단정 대신 **질문 형태**("X 경로에서 동시 접근 가능성이 있는데 Y 락으로 보장되나요?")로 제시한다. 거짓 🔴 한 건이 리뷰 신뢰도를 무너뜨린다. **확인된 것만 🔴.**
   - **근거 동봉**: 🔴 및 버그성 🟡 finding 은 확인에 쓴 코드 근거(파일:라인, 락/타입/수명 사실)를 finding 본문에 함께 적는다 — "왜 진짜 버그인지"가 코드로 드러나야 한다.
