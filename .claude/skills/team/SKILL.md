...
- 단일 파일의 작은 수정, 작업 간 의존성이 강해 병렬화가 오히려 위험한 경우에는
  team 모드를 사용하지 않는다.
...
- team 모드는 기본적으로 현재 top-level 사용자 요청에만 살아 있는 임시 팀이다.
  ...
  TeamDelete로 정리한다. 정리 시점 판단은 `팀 정리` 섹션을 따른다 — 사용자와의
  대화가 이어지는 중이면 후속 요청 가능성도 함께 고려한다.
- 사용자가 "팀 유지", "keep alive", "persistent team", "다음 작업에도 재사용"처럼
  명시한 경우에만 요청 종료 후 팀을 유지한다.
- **`Agent` 도구로 새 teammate를 생성하기 직전에 반드시 `ListAgents`로 현재 팀원
  목록과 상태를 확인한다.** 이 확인을 건너뛴 신규 생성은 규칙 위반으로 본다.
  확인 결과 재사용 가능한 teammate가 있으면 재브리핑해 재사용하고, 신규 생성은
  재사용이 불가능한 이유(역할 불일치, ownership 충돌, 독립 판단 필요 등)를 댈 수
  있을 때만 한다. 신규 생성한 경우 사용자 응답에 **왜 재사용하지 않고 새로
  만드는지 한 줄로 밝힌다.**
- 메인 에이전트를 포함해 유지하거나 동시에 활동하는 에이전트는 최대 5개로 제한한다.
  즉 teammate는 최대 4명까지만 사용한다. 스폰 직전 `ListAgents` 결과로 **현재
  teammate 수를 세고, 새로 만들어도 4명을 넘기지 않는지 확인**한다.
- team 모드를 사용하면 메인 에이전트의 기본 역할은 사용자 응답, 조율, 모니터링,
  최종 통합이다. 사용자의 추가 질문을 계속 받을 수 있도록 독립 subtask는 가능한
  한 teammate에게 위임한다.
- 메인 에이전트는 teammate 배정 전 필요한 탐색, ownership 충돌이 없는 짧은
  보조 작업, 최종 통합/검증, 사용자가 메인 직접 처리를 요청한 경우에만 subtask를
  직접 맡는다.
- 팀이 살아 있는 동안 사용자가 추가 요청을 보내면 **idle teammate 위임이
  기본값**이다. 메인은 매 추가 요청마다 직접 구현하기 전에 TaskList/pane 상태로
  idle teammate 유무를 먼저 확인하고, 파일 수정·조사·검증이 포함된 요청은
  TaskCreate 후 idle teammate에게 SendMessage로 배정한다. 메인 직접 처리는
  한두 문장으로 끝나는 답변, 상태 보고, ownership/backlog 조정으로 한정한다.
  "간단해 보여서 직접 처리"는 허용하지 않는다 — 파일을 수정하는 요청이면 크기와
  무관하게 위임을 기본으로 한다. idle teammate가 없으면 backlog에 넣고 가장
  먼저 완료되는 teammate에게 배정하되, 사용자가 즉시 처리를 명시한 경우에만
  메인이 직접 맡는다.
- 이미 teammate가 4명이거나 그 이상이면 새 teammate를 만들지 않는다. 초과 상태를
  발견하면 새 작업 배정보다 정리를 먼저 한다. 이번 작업에는 최대 4명만 선택하고,
  초과 idle teammate는 정리한다.
- 이전 요청에서 남은 idle/stale 팀이나 pane이 있으면 진행 중 task가 없는지 먼저
  확인한다. 같은 작업 영역의 연속 요청이면 그 컨텍스트를 가진 teammate를 재브리핑해
  재사용하는 것을 우선하고, 작업 영역이 바뀌었으면 재사용하지 않고 먼저 정리한다.
  진행 중 task가 있으면 사용자에게 남은 작업을 보고하고 새 작업에 재배정하지 않는다.
...
**ListAgents**, **TaskList**, **TaskGet**, tmux pane 상태를 확인한다. running
task가 없고 작업 영역도 바뀌었으면 **TeamDelete**로 정리하고 새 **TeamCreate**를
실행한다 (team_name 지정). 같은 작업 영역의 연속 요청이면 정리보다 재브리핑
재사용을 먼저 검토한다. running task가 있으면 해당 teammate를 새 작업에 재사용하지
않고 사용자에게 남은 작업을 보고한다.
...
| failed | 직전 task가 실패했거나 오류 상태 | 원인 확인 전 재사용하지 않음. 원인이 외부 요인이고 해소가 확인되면 재브리핑 후 재사용 |
...
failed teammate는 실패 원인을 구분해 처리한다. 원인이 **작업 내용과 무관한 외부
요인**(세션/사용량 한도, 일시적 네트워크·MCP 오류, 런타임 중단 등)이고 그 원인이
**해소된 것이 확인되면**, 이전 작업 맥락을 폐기하라는 지시와 함께 재브리핑해
재사용할 수 있다 — 한도 리셋 후의 teammate는 정상적인 재사용 후보다. 반면 **작업
자체가 실패한 경우**(잘못된 수정, 반복 오류, 지시 불이행)는 원인이 정리되기 전까지
재사용하지 않는다.
...
5. idle teammate가 부족하거나 역할상 재사용이 부적절한 경우에만 **Agent**로 새
   teammate를 생성한다. 호출 **직전에 `ListAgents`로 팀원 목록·상태·인원수를
   확인**해 재사용 가능한 teammate가 없다는 것과 생성 후에도 teammate 4명을 넘지
   않는다는 것을 확인한다 — `team_name`과 `name` 파라미터를 반드시 지정해야 tmux
   pane에 표시된다
...
사용자가 작업 중 보내는 추가 요청도 backlog에 들어가는 새 subtask로 취급한다.
`작업 방침`의 추가 요청 위임 규칙에 따라 idle teammate에게 우선 배정하고, 메인이
직접 처리한 경우에는 위임하지 않은 이유를 응답에 한 줄로 명시한다.
...
- 직전 작업 자체가 실패했고 실패 원인이 아직 정리되지 않은 경우 (실패 원인이
  세션/사용량 한도, 일시적 네트워크·MCP 오류처럼 작업과 무관한 외부 요인이고 해소가
  확인된 경우는 제외 — 재브리핑 후 재사용 가능)
...
완료 보고를 받으면 메인은 **브리프의 모든 항목이 보고에 포함됐는지 대조**하고,
누락 항목이 있으면 같은 teammate에게 잔여 항목만 다시 배정한다. idle 알림만 오고
완료 보고가 없으면 `rg`/`git diff`로 **실제 파일 상태를 직접 확인**한 뒤 미착수면
재요청한다.
...
정리 시점은 대화 흐름을 보고 판단한다. 요청이 끝났더라도 **사용자와의 대화가
이어지는 중**이고 같은 작업 영역의 후속 요청이 올 가능성이 있으면, 그 영역 컨텍스트를
가진 teammate는 곧바로 종료하지 않고 idle로 두는 편이 재사용 가치가 높다. 반대로
작업 영역이 종료됐거나 사용자가 다른 주제로 넘어갔거나 대화가 마무리된 것으로
보이면 기본값대로 정리한다 — 명시적 keep-alive 요청이 없는 팀을 무한정 유지하지는
않는다. 이미 정리한 뒤 새 요청이 도착한 경우에도 `작업 방침`의 **스폰 전
`ListAgents` 확인**을 수행한 뒤 생성 여부를 결정한다.
