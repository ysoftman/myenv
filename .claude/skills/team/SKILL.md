---
name: team
description: Start a temporary agent team in tmux panes for collaborative work, with at most 5 total agents including the main agent. Use proactively when a task can be split across independent files, modules, or concerns. Also trigger when the user asks to work on multiple things at once, speed up a large task, or mentions "팀", "병렬", "parallel", "동시에", "나눠서".
...
- team 모드를 사용하면 메인 에이전트도 작업자다. 초기 배정 직후 메인은 조율만
  하며 기다리지 말고, ownership이 겹치지 않는 독립 subtask 하나를 직접 수행한다.
...
- subtask 수가 많아도 이번 작업에 배정하는 teammate는 최대 4명으로 제한한다.
  ...
  재배정한다. 메인 에이전트도 backlog에서 독립 subtask 하나를 맡는다.
...
초기 teammate 배정이 끝나면 메인 에이전트도 즉시 자신의 독립 subtask를 수행한다.
메인은 아래 경우에만 조율로 돌아온다.

- teammate가 완료되어 다음 subtask를 재배정해야 하는 경우
- 실패, 충돌, 의존성 변경 때문에 backlog를 재계획해야 하는 경우
- 모든 running/queued subtask가 끝나 최종 통합이 필요한 경우
