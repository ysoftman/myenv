...
- 코드 변경으로 바로 고칠 수 있는 코멘트는 가능한 한 GitHub suggestion 코드 블록을 포함한다. suggestion은 코멘트가 달릴 diff 라인과 정확히 대응해야 하며, 범위가 맞지 않거나 여러 파일 수정이 필요한 경우에는 일반 코드 블록이나 설명으로 대체한다.
