...
- 위임은 **항상 `Agent` 도구로** 한다. 서브에이전트는 기본적으로 백그라운드로 돌고 완료 시 알림이 오므로 `run_in_background` 를 명시하지 않는다 — 이 파라미터는 버전/컨텍스트에 따라 제공되지 않으며, 없는데 넘기면 호출이 거부된다. 메인은 호출 직후 사용자 입력 대기/처리로 돌아가고, 완료 알림이 오면 후처리(결과 보고, task 등록/완료)만 진행한다.
...
4. **`Agent` 도구로 `reviewer` 서브에이전트를 디스패치**한다(백그라운드가 기본). 변경이 크면(변경 파일 5개 이상 또는 500라인 이상) 파일을 모듈/디렉터리 단위로 2~4그룹으로 나눠 **그룹별 `reviewer` 에이전트를 한 메시지에서 동시에 디스패치**한다 — 각 prompt에는 해당 그룹의 diff만 넣되, 상호작용 파악용으로 전체 변경 파일 목록을 함께 전달한다. 관련 파일(같은 모듈/디렉터리)은 같은 그룹에 묶는다. 그 미만 규모면 기존대로 1개만 디스패치한다(분할 오버헤드가 더 크다). 호출 직후 메인은 사용자 입력 대기/처리로 돌아간다(리뷰 완료를 동기적으로 기다리지 않는다). prompt에 다음을 포함:
   - PR 번호와 제목
   - 변경된 diff 전체
   - PR 본문/설명 (의도 컨텍스트)
   ...
   - **`reviewer` 방법론의 보고 기준(Methodology #4·#7)을 따르도록 지시**한다: 모든 finding 에 실패 시나리오 필수, 각 finding 에 `확정`/`추정` verdict 라벨, `확정` 은 검증 게이트(hunk 밖 실제 head 코드로 타입·락·수명·참조 확인) 통과분만, 확신 없으면 `추정` + 질문 형태, 🟢 는 기본 미생성, 상세는 최대 8개(정확성 우선)이고 초과분은 `그 외 🟡 N건` 한 줄 요약. 사용자가 "꼼꼼히"/"전체" 리뷰를 요청한 경우에만 "🟢 포함"을 prompt 에 명시한다. (기준 자체는 `reviewer` 단일 출처)
...
5. 서브에이전트 완료 알림이 오면 반환된 리뷰 결과(🔴 높음 / 🟡 중간 + `확정`/`추정` verdict + PR 변경 영향이 아닌 것만 `[PR 무관]`으로 표시된 findings)를 받는다. 분할 디스패치한 경우 **모든 그룹 완료 후 findings 를 병합**하고, 동일 `파일:라인`의 중복 finding 은 우선순위가 높은 쪽 하나만 남긴다. 병합 후에도 상세는 8개를 넘기지 않는다 — `reviewer` 의 정렬 기준(정확성 > 품질, 🔴 > 🟡, `확정` > `추정`)으로 8개만 남기고 나머지는 `그 외 🟡 N건 (요청 시 표시)` 한 줄로 요약한다
...
3. 사용자가 승인하면 **`Agent` 도구로 `reviewer` 서브에이전트를 호출**해 코멘트 작성을 넘긴다(백그라운드가 기본). 호출 직후 메인은 즉시 사용자 입력 대기/처리로 돌아간다(작성 완료를 동기적으로 기다리지 않는다).
