...
5. 서브에이전트 완료 알림이 오면 반환된 리뷰 결과(🔴 높음 / 🟡 중간 / 🟢 낮음 + PR 변경 영향이 아닌 것만 `[PR 무관]`으로 표시된 findings)를 받는다. 분할 디스패치한 경우 **모든 그룹 완료 후 findings 를 병합**하고, 동일 `파일:라인`의 중복 finding 은 우선순위가 높은 쪽 하나만 남긴다
