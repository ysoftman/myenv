...
mpkgs["feroxbuster"]="feroxbuster"         # (https://github.com/epi052/feroxbuster) A fast, simple, recursive content discovery tool, 웹 애플리케이션의 숨겨진 디렉터리,파일,엔드포인트를 빠르게 탐색(bruteforce), SecLists(보안 테스트,펜테스트, CTF, 취약점 점검에 쓰이는 워드리스트집)설치 필요 git clone --depth 1 https://github.com/danielmiessler/SecLists.git ~/workspace/SecLists; feroxbuster -u http://argocd.ysoftman/ -w ~/workspace/SecLists/Discovery/Web-Content/raft-medium-directories.txt
