...
download_ysoftman_youtube_music() {
    ...
    playlists=(
        "https://www.youtube.com/playlist?list=PLxZefZxz0kXuZSbU4AQsIiES32Ddqbwbe"
        "https://www.youtube.com/playlist?list=PLxZefZxz0kXva-lM3tLP49r5zZQKC_w5C"
        "https://www.youtube.com/playlist?list=PLxZefZxz0kXt-LqxE7I2IJERF8-t_8FUT"
        "https://www.youtube.com/playlist?list=PLxZefZxz0kXt0zWX9cy0Gf5DcikcA4S01"
    )
    for pl in "${playlists[@]}"; do
        # download-archive 파일에 이미 다운로드된 음악 hash 가 기록되어 이를 참고하면 중복 다운로드를 피할 수 있다.
        # --restrict-filenames 옵션으로 디렉토리,파일명에 아스키만 사용하도로하면 한글이름이 다 빠져 사용하지 않는다.
        yt-dlp "$pl" \
